    ...
    <>
      ...
      <main>
        ...
        <ResearchArea subtitle='Verifiable Delay Functions and Random Beacons' mb={10}>
          ...
          <Text mb={4} fontSize='sm'>
              <em>
                  We present <InlineMath math={'\\mathsf{Bingo}'} />, an adaptively secure and optimally resilient packed asynchronous verifiable secret sharing (PAVSS) protocol that allows a dealer to share <InlineMath math={'f+1'} /> secrets with a total communication complexity of <InlineMath math={'O(\\lambda n^2)'} /> words, where <InlineMath math={'\\lambda'} /> is the security parameter and <InlineMath math={'n'} /> is the number of parties.
              </em>
          </Text>

          <Text mb={4} fontSize='sm'>
              <em>
                  Using <InlineMath math={'\\mathsf{Bingo}'} />, we obtain an adaptively secure validated asynchronous Byzantine agreement (VABA) protocol that uses <InlineMath math={'O(\\lambda n^3)'} /> expected words and constant expected time, which we in turn use to construct an adaptively secure high-threshold asynchronous distributed key generation (ADKG) protocol that uses <InlineMath math={'O(\\lambda n^3)'} /> expected words and constant expected time.
              </em>
          </Text>
...
>
        ...
        <Publication
            ...
            <Text mb={4} fontSize='sm'>
            <em>
            Our protocol supports <i>commit-and-prove expansions</i>: the prover selects the subtable containing the elements used in the lookup, that is unknown to the verifier, commits to it and later prove relation with the committed element. This feature makes "Baloo" especially suitable for prover input-output relations on hash functions, and in particular to instantiate the Ethereum Virtual Machine (EVM).
            </em>
            </Text>
          ...
          <Publication
            ...
            {<InlineMath math={'H \\subset \\mathbb{F}'} />} of size {<InlineMath math={'m'} />} are contained in a
            table {<InlineMath math={'T\\in \\mathbb{F}^N'} />}. After a preprocessing step, the prover algorithm runs in time{' '}
            {<InlineMath math={'O(m\\log ^2 m)'} />}.
            This improves a recent result of Caulk+[PK22] for the same problem with run time{' '}
            {<InlineMath math={'O(m^2)'} />}, that in turn improved another recent result with run time{' '}
            {<InlineMath math={'O(m^2+m\\log N)'} />} Caulk[ZBK+22]. We pose further improving this complexity to{' '}
            {<InlineMath math={'O(m\\log m)'} />} as the next important milestone for efficient zk-SNARK lookups.
            </em>
            </Text>
          </Publication>
          ...
          <Publication
            ...
            <Text mb={4} fontSize='sm'>
            <em>
            We present <i>position-hiding linkability</i> for vector commitment schemes: one can prove in zero knowledge that one or {<InlineMath math={'m'} />} values that comprise commitment {<InlineMath math={'\\textsf{cm}'} />} all belong to the vector of size {<InlineMath math={'N'} />} committed to in {<InlineMath math={'\\C'} />}. Our construction {<InlineMath math={'\\textsf{Caulk}'} />} can be used for membership proofs and lookup arguments and outperforms all existing alternatives in prover time by orders of magnitude.
            </em>
            </Text>

            <Text mb={4} fontSize='sm'>
            <em>
            For both single- and multi-membership proofs the {<InlineMath math={'\\textsf{Caulk}'} />} protocol beats SNARKed Merkle proofs by the factor of 100 even if the latter is instantiated with Poseidon hash. Asymptotically our prover needs {<InlineMath math={'O(m^2 + m\\log N)'} />} time to prove a batch of {<InlineMath math={'m'} />} openings, whereas proof size is {<InlineMath math={'O(1)'} />} and verifier time is {<InlineMath math={'O(\\log(\\log N))'} />}.
            </em>
            </Text>

            <Text mb={4} fontSize='sm'>
            <em>
            As a lookup argument, {<InlineMath math={'\\textsf{Caulk}'} />} is the first scheme with prover time sublinear in the table size, assuming {<InlineMath math={'O(N\\log N)'} />} preprocessing time and {<InlineMath math={'O(N)'} />} storage. It can be used as a subprimitive in verifiable computation schemes in order to drastically decrease the lookup overhead.
            </em>
            </Text>
          ...
          >
            ...
            <Text mb={4} fontSize='sm'>
              <em>
                We design and implement <InlineMath math={'\\mathsf{SNARKBlock}'} />, a new protocol for zero-knowledge blocklisting with server-side verification that is logarithmic in the size of the blocklist. <InlineMath math={'\\mathsf{SNARKBlock}'} /> is also the first approach to support ad-hoc, federated blocklisting: websites can mix and match their own blocklists from other blocklists and dynamically choose which identity providers they trust.
              </em>
            </Text>

            <Text mb={4} fontSize='sm'>
              <em>
                Our core technical advance, of separate interest, is the <InlineMath math={'\\mathsf{HICIAP}'} /> zero-knowledge proof system, which addresses a common problem in privacy-preserving protocols: using zero-knowledge proofs for repeated but unlinkable interactions. Rerandomzing a Groth16 proof achieves unlinkability without the need to recompute the proof for every interaction. But this technique does not apply to applications where each interaction includes multiple Groth16 proofs over a common hidden input (e.g., the user's identity). Here, the best known approach is to commit to the hidden input and feed it to each proof, but this creates a persistent identifier, forcing recomputation. <InlineMath math={'\\mathsf{HICIAP}'} /> resolves this problem by aggregating <InlineMath math={'n'} /> Groth16 proofs into one <InlineMath math={'O(\\log n)'} />-sized, <InlineMath math={'O(\\log n)'} />-verification time proof which also shows that the input proofs share a hidden input. Because <InlineMath math={'\\mathsf{HICIAP}'} /> is zero-knowledge, repeated shows of the same aggregate or an updated aggregate are unlinkable even though the underlying Groth16 proofs are never recomputed.
              </em>
            </Text>
          </Publication>
        ...
        <ResearchArea subtitle='Hash Functions' mb={10}>
          ...
          >
            ...
            <Text mb={4} fontSize='sm'>
              <em>
               In this paper, we focus on lookup-based computations, a specific component which allows to verify that a particular witness is contained in a lookup table. We work over 31-bit and 64-bit finite fields <InlineMath math={'\\mathbb{F}_p'} />
                , both of which are used in various modern proof systems today and allow for fast implementations. We propose a new 2-to-1 compression function and a SAFE hash function, instantiated by the Monolith permutation. The permutation is significantly more efficient than its competitors, both in terms of circuit friendliness and plain performance, which has become one of the main bottlenecks in various use cases. This includes Reinforced Concrete and Tip5, the first two hash functions using lookup computations internally. Moreover, in Monolith we instantiate the lookup tables as functions defined over <InlineMath math={'\\mathbb{F}_2'} />
                while ensuring that the outputs are still elements in <InlineMath math={'\\mathbb{F}_p'} />. Contrary to Reinforced Concrete and Tip5, this approach allows efficient constant-time plain implementations which mitigates the risk of side-channel attacks potentially affecting competing lookup-based designs. Concretely, our constant time 2-to-1 compression function is faster than a constant time version of Poseidon2 by a factor of 7. Finally, it is also the first arithmetization-oriented function with a plain performance comparable to SHA3-256, essentially closing the performance gap between circuit-friendly hash functions and traditional ones.
              </em>
            </Text>
          </Publication>
        ...
        <ResearchArea subtitle='Threshold Cryptography' mb={10}>
          ...
          >
              <Text mb={4} fontSize='sm'>
                  <em>
                      Given {<InlineMath math={'\\ell'} />} parties with sets {<InlineMath math={'X_1, \\ldots, X_{\\ell}'} />} of size {<InlineMath math={'n'} />}, we would like to securely compute the intersection {<InlineMath math={'\\cap_{i=1}^{\\ell} X_i'} />}, if it is larger than {<InlineMath math={'n-t'} />} for some threshold {<InlineMath math={'t'} />}, without revealing any other additional information. It has previously been shown (Ghosh and Simkin, Crypto 2019) that this function can be securely computed with a communication complexity that only depends on {<InlineMath math={'t'} />} and in particular does not depend on {<InlineMath math={'n'} />}. For small values of {<InlineMath math={'t'} />}, this results in protocols that have a communication complexity that is sublinear in the size of the inputs. Current protocols either rely on fully homomorphic encryption or have an at least quadratic dependency on the parameter {<InlineMath math={'t'} />}.
                  </em>
              </Text>

              <Text mb={4} fontSize='sm'>
                  <em>
                      In this work, we construct protocols with a quasilinear dependency on {<InlineMath math={'t'} />} from simple assumptions like additively homomorphic encryption and oblivious transfer. All existing approaches, including ours, rely on protocols for computing a single bit, which indicates whether the intersection is larger than {<InlineMath math={'n-t'} />} without actually computing it. Our key technical contribution, which may be of independent interest, takes any such protocol with secret shared outputs and communication complexity {<InlineMath math={'\\mathcal{O}(\\lambda \\ell \\cdot \\mathsf{poly}(t))'} />}, where {<InlineMath math={'\\lambda'} />} is the security parameter, and transforms it into a protocol with communication complexity {<InlineMath math={'\\mathcal{O}(\\lambda^2 \\ell t \\cdot \\mathsf{polylog}(t))'} />}.
                  </em>
              </Text>
          </Publication>
          ...
          >
              <Text mb={4} fontSize='sm'>
                  <em>
                      Threshold secret sharing allows a dealer to split a secret {<InlineMath math={'s'} />} into {<InlineMath math={'n'} />} shares, such that any {<InlineMath math={'t'} />} shares allow for reconstructing {<InlineMath math={'s'} />}, but no {<InlineMath math={'t-1'} />} shares reveal any information about {<InlineMath math={'s'} />}. Leakage-resilient secret sharing requires that the secret remains hidden, even when an adversary additionally obtains a limited amount of leakage from every share.
                  </em>
              </Text>

              <Text mb={4} fontSize='sm'>
                  <em>
                      Benhamouda et al. (CRYPTO'18) proved that Shamir's secret sharing scheme is one bit leakage-resilient for reconstruction threshold {<InlineMath math={'r \\geq 0.85n'} />} and conjectured that the same holds for {<InlineMath math={'t = cn'} />} for any constant {<InlineMath math={'0 \\leq c \\leq 1'} />}. Nielsen and Simkin (EUROCRYPT'20) showed that this is the best one can hope for by proving that Shamir's scheme is not secure against one-bit leakage when {<InlineMath math={'t = c n / \\log n'} />}.
                  </em>
              </Text>

              <Text fontSize='sm'>
                  <em>
                      In this work, we strengthen the lower bound of Nielsen and Simkin. We consider noisy leakage-resilience, where a random subset of leakages is replaced by uniformly random noise. We prove a lower bound for Shamir's secret sharing, similar to that of Nielsen and Simkin, which holds even when a constant fraction of leakages is replaced by random noise. To this end, we first prove a lower bound on the share size of any noisy-leakage-resilient sharing scheme. We then use this lower bound to show that there exist universal constants {<InlineMath math={'c_1'} />}, {<InlineMath math={'c_2'} />}, such that for infinitely many n, it holds that Shamir's secret sharing scheme is not noisy-leakage-resilient for {<InlineMath math={'t \\leq c_1 n / \\log n'} />}, even when a {<InlineMath math={'c_2'} />} fraction of leakages are replaced by random noise.
                  </em>
              </Text>
          </Publication>
        ...
        <ResearchArea subtitle='Lattice Cryptography' mb={10}>
          ...
          >
              <Text mb={4} fontSize='sm'>
                  <em>
                      ...
                      We present Squirrel, a concretely efficient lattice-based multi-signature scheme in the synchronized setting that works for a bounded number of {<InlineMath math={'2^{\\tau}'} />} time steps and allows for aggregating up to {<InlineMath math={'\\rho'} />} signatures at each step, where both {<InlineMath math={'\\tau'} />} and {<InlineMath math={'\\rho'} />} are public parameters upon which the efficiency of our scheme depends. Squirrel allows for non-interactive aggregation of independent signatures and is proven secure in the random oracle model in the presence of rogue-key attacks assuming the hardness of the short integer solution problem in a polynomial ring.

                      We provide a careful analysis of all parameters and show that Squirrel can be instantiated with good concrete efficiency. For {<InlineMath math={'\\tau = 24'} />} and {<InlineMath math={'\\rho = 4096'} />}, a signer could sign a new message every 10 seconds for 5 years non-stop. Assuming the signer has a cache of 112 MB, signing takes 68 ms and verification of an aggregated signature takes 36 ms. The size of the public key is 1 KB, the size of an individual signature is 52 KB, and the size of an aggregated signature is 771 KB.
                  </em>
              </Text>
          </Publication>
          ...
          >
              <Text mb={4} fontSize='sm'>
                  <em>
                      Property-preserving hash functions allow for compressing long inputs {<InlineMath math={'x_0'} />} and {<InlineMath math={'x_1'} />} into short hashes {<InlineMath math={'h(x_0)'} />} and {<InlineMath math={'h(x_1)'} />} in a manner that allows for computing a predicate {<InlineMath math={'P(x_0, x_1)'} />} given only the two hash values without having access to the original data. Such hash functions are said to be adversarially robust if an adversary that gets to pick {<InlineMath math={'x_0'} />} and {<InlineMath math={'x_1'} />} after the hash function has been sampled, cannot find inputs for which the predicate evaluated on the hash values outputs the incorrect result.
                  </em>
              </Text>

              <Text mb={4} fontSize='sm'>
                  <em>
                      In this work, we construct robust property-preserving hash functions for the hamming-distance predicate which distinguishes inputs with a hamming distance at least some threshold {<InlineMath math={'t'} />}. The security of the construction is based on standard lattice hardness assumptions. Our construction has several advantages over the best known previous construction by Fleischhacker and Simkin (Eurocrypt 2021). Our construction relies on a single well-studied hardness assumption from lattice cryptography whereas the previous work relied on a newly introduced family of computational hardness assumptions.
                  </em>
              </Text>

              <Text mb={4} fontSize='sm'>
                  <em>
                      In terms of computational effort, our construction only requires a small number of modular additions per input bit, whereas the work of Fleischhacker and Simkin required several exponentiations per bit as well as the interpolation and evaluation of high-degree polynomials over large fields. An additional benefit of our construction is that the description of the hash function can be compressed to {<InlineMath math={'\\lambda'} />}. Previous work has descriptions of length {<InlineMath math={'\\mathcal{O}(\\ell \\lambda)'} />} bits for input bit-length {<InlineMath math={'\\ell'} />}.
                  </em>
              </Text>
          ...
          >
              <Text mb={4} fontSize='sm'>
                  <em>
                      In this paper, we study the <i>hybrid dual attack</i> over Learning with Errors (LWE) problems for <i>any</i> secret distribution. Prior to our work, hybrid attacks are only considered for sparse and/or small secrets. A new and interesting result from our analysis shows that for most cryptographic use cases a hybrid dual attack outperforms a standalone dual attack, regardless of the secret distribution. We formulate our results into a framework of predicting the performance of the hybrid dual attacks. We also present a few tricks that further improve our attack. To illustrate the effectiveness of our result, we re-evaluate the security of <i>all</i> LWE related proposals in round 3 of NIST's post-quantum cryptography process, and improve the state-of-the-art cryptanalysis results by 2-14 bits, under the BKZ-core-SVP model.
                  </em>
              </Text>
          </Publication>
        ...
        <ResearchArea subtitle='Data Structures' mb={10}>
          ...
          >
              <Text mb={4} fontSize='sm'>
                  <em>
                      In this work we study Invertible Bloom Lookup Tables (IBLTs) with small failure probabilities. IBLTs are highly versatile data structures that have found applications in set reconciliation protocols, error-correcting codes, and even the design of advanced cryptographic primitives. For storing {<InlineMath math={'n'} />} elements and ensuring correctness with probability at least {<InlineMath math={'1 - \\delta'} />}, existing IBLT constructions require {<InlineMath math={'\\Omega( n( \\frac{ \\log 1/\\delta}{\\log n} + 1) )'} />} space and they crucially rely on fully random hash functions.
                  </em>
              </Text>

              <Text mb={4} fontSize='sm'>
                  <em>
                      We present new constructions of IBLTs that are simultaneously more space efficient and require less randomness. For storing {<InlineMath math={'n'} />} elements with a failure probability of at most {<InlineMath math={'\\delta'} />}, our data structure only requires {<InlineMath math={'\\mathcal{O}( n + \\log(1 / \\delta) \\log\\log(1 / \\delta) )'} />} space and {<InlineMath math={'\\mathcal{O}( \\log( \\log( n ) / \\delta ) )'} />}-wise independent hash functions.
                  </em>
              </Text>

              <Text mb={4} fontSize='sm'>
                  <em>
                      As a key technical ingredient we show that hashing {<InlineMath math={'n'} />} keys with any {<InlineMath math={'k'} />}-wise independent hash function {<InlineMath math={'h: U \\mapsto [Cn]'} />} for some sufficiently large constant {<InlineMath math={'C'} />} guarantees with probability {<InlineMath math={'1 - 2^{-\\Omega(k)}'} />} that at least {<InlineMath math={'n / 2'} />} keys will have a unique hash value. Proving this is highly non-trivial as {<InlineMath math={'k'} />} approaches {<InlineMath math={'n'} />}. We believe that the techniques used to prove this statement may be of independent interest.
                  </em>
              </Text>
          </Publication>
          ...
          >
              ...
              <Text mb={4} fontSize='sm'>
                  <em>
                      Concretely, assume one is given a vector {<InlineMath math={'(m_1, \\ldots, m_n)'} />} with at most {<InlineMath math={'t'} />} distinct indices {<InlineMath math={'i \\in [n]'} />}, such that {<InlineMath math={'m_i \\neq 0'} />} and assume {<InlineMath math={'(\\mathsf{Gen}, \\mathsf{Enc}, \\mathsf{Dec})'} />} is an additively homomorphic encryption scheme. The authors show that one can compress {<InlineMath math={'(\\mathsf{Enc}(k, m_1), \\ldots, \\mathsf{Enc}(k, m_n))'} />}, without knowing the secret key {<InlineMath math={'k'} />}, into a digest with size dependent on the upper bound on the sparsity {<InlineMath math={'t'} />}, but not on the total vector length {<InlineMath math={'n'} />}. 
                  </em>
              </Text>
          ...
          >
              <Text mb={4} fontSize='sm'>
                  <em>
                      We study the task of obliviously compressing a vector comprised of {<InlineMath math={'n'} />} ciphertexts of size {<InlineMath math={'\\xi'} />} bits each, where at most {<InlineMath math={'t'} />} of the corresponding plaintexts are non-zero. This problem commonly features in applications involving encrypted outsourced storages, such as searchable encryption or oblivious message retrieval. We present two new algorithms with provable worst-case guarantees, solving this problem by using only homomorphic additions and multiplications by constants. Both of our new constructions improve upon the state of the art asymptotically and concretely.
                  </em>
              </Text>

              <Text mb={4} fontSize='sm'>
                  <em>
                      Our first construction, based on sparse polynomials, is perfectly correct and the first to achieve an asymptotically optimal compression rate by compressing the input vector into {<InlineMath math={'\\mathcal{O}(t \\xi)'} />}. Compression can be performed homomorphically by performing {<InlineMath math={'\\mathcal{O}( n \\log n)'} />} homomorphic additions and multiplications by constants. The main drawback of this construction is a decoding complexity of {<InlineMath math={'\\Omega(\\sqrt{n})'} />}.
                  </em>
              </Text>

              <Text mb={4} fontSize='sm'>
                  <em>
                      Our second construction is based on a novel variant of invertible bloom lookup tables and is correct with probability {<InlineMath math={'1 - 2^{-\\kappa}'} />}. It has a slightly worse compression rate compared to our first construction as it compresses the input vector into {<InlineMath math={'\\mathcal{O}(\\xi\\kappa t / \\log t)'} />} bits, where {<InlineMath math={'\\kappa \\geq \\log t'} />}. In exchange, both compression and decompression of this construction are highly efficient. The compression complexity is dominated by {<InlineMath math={'\\mathcal{O}(n \\kappa / \\log t)'} />} homomorphic additions and multiplications by constants. The decompression complexity is dominated by {<InlineMath math={'\\mathcal{O}(\\kappa t / \\log t )'} />} decryption operations and equally many inversions of a pseudorandom permutation.
                  </em>
              </Text>
          </Publication>
        ...
        <ResearchArea subtitle='Miscellaneous'>
          ...
          >
              <Text mb={4} fontSize='sm'>
                  <em>
                      ...
                      Laconic PSI can be useful in applications, where servers with large sets would like to learn the intersection of their set with smaller sets owned by resource-constrained clients and where multiple rounds of interactions are not possible.
                  </em>
              </Text>

              <Text mb={4} fontSize='sm'>
                  <em>
                      Previously, practically relevant laconic PSI protocols were only known from factoring-type assumptions. The contributions of this work are twofold:
                      1) We present the first laconic PSI protocol based on assumptions over pairing-friendly elliptic curves; and 2) For the first time we provide empirical evaluation of any laconic PSI protocol by carefully implementing and optimising both our and previous protocols. Our experimental results show that our protocol outperforms prior laconic PSI protocols.
                  </em>
              </Text>
          </Publication>
          ...
          >
              <Text mb={4} fontSize='sm'>
                  <em>
                      We study mix-nets in the context of cryptocurrencies. Here we have many computationally weak shufflers that speak one after another and want to jointly shuffle a list of ciphertexts {<InlineMath math={'(c_1, \\ldots, c_n)'} />}. Each shuffler can only permute {<InlineMath math={'k \\ll n'} />} ciphertexts at a time. An adversary {<InlineMath math={'\\mathcal{A}'} />} can track some of the ciphertexts and adaptively corrupt some of the shufflers.
                  </em>
              </Text>
          ...
          >
              <Text mb={4} fontSize='sm'>
                  <em>
                      Interactive Non-Malleable Codes were introduced by Fleischhacker et al. (TCC 2019) in the two party setting with synchronous tampering. The idea of this type of non-malleable code is that it "encodes" an interactive protocol in such a way that, even if the messages are tampered with according to some class {<InlineMath math={'\\mathcal{F}'} />}, the result of the execution will either be correct, or completely unrelated to the inputs of the participating parties. In the synchronous setting the adversary is able to modify the messages being exchanged but cannot drop messages nor desynchronize the two parties by first running the protocol with the first party and then with the second party. In this work, we define interactive non-malleable codes in the non-synchronous multi-party setting and construct such interactive non-malleable codes for the class {<InlineMath math={'\\mathcal{F}^s_{\\mathsf{bounded}}'} />}. The construction is applicable to any multi-party protocol with a fixed message topology.
                  </em>
              </Text>
          </Publication>
          ...
          <Publication
           ...
           <Text mb={4} fontSize='sm'>
             <em>
               In this paper, we analyze high-degree functions such as the Legendre symbol or the modulo-2 operation as building blocks for the nonlinear layer of a cryptographic scheme over {<InlineMath math={"\\mathbb{F}_p^n"} />}. Our focus regards the security analysis rather than the efficiency in the mentioned use cases. For this purpose, we present several new invertible functions that make use of the Legendre symbol or of the modulo-2 operation.
             </em>
            </Text>
